...
//groove utility - global geometry storage
unlekker.modelbuilder.UVertexList recordPerimeterUpper,recordPerimeterLower,recordHoleUpper,recordHoleLower;//storage for perimeter and center hole of record
unlekker.modelbuilder.UVertexList grooveOuterUpper,grooveOuterLower,grooveInnerUpper,grooveInnerLower;//groove vertices
unlekker.modelbuilder.UVertexList lastEdge;//storage for conecting one groove to the next
...
void completeGrooveRev(int grooveNum, float radius, float[] audioData){
  ...
  if (grooveNum==0){//if joining a groove to the edge of the record
    ...
    unlekker.modelbuilder.UVertexList _first_grooveInnerUpper = grooveInnerUpper;
    grooveOuterUpper.add(_first_grooveInnerUpper);
  }
...
public unlekker.modelbuilder.UVertexList beginStartCap(float radius, float firstSample){//this is a tiny piece of geometry that closes off the front end of the groove
  unlekker.modelbuilder.UVertexList stop1 = new unlekker.modelbuilder.UVertexList();
  unlekker.modelbuilder.UVertexList stop2 = new unlekker.modelbuilder.UVertexList();
...
void finishStartCap(float radius, unlekker.modelbuilder.UVertexList stop1){
  unlekker.modelbuilder.UVertexList stop2 = new unlekker.modelbuilder.UVertexList();
...
float drawPenultGroove(float radius,int grooveNum, float[] audioData, float radIncr){
  ...
  unlekker.modelbuilder.UVertexList ridge = new unlekker.modelbuilder.UVertexList();
  float theta;
  ...
  float theta_index = 0.0f;
  for(theta_index=theta;theta_index<TWO_PI;theta_index+=incrNum){//for theta between current position and 2pi
    sineTheta = sin(theta);
